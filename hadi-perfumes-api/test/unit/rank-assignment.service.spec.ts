...
describe('RankAssignmentService (Unit)', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(RankAssignment),
          useValue: mockRankAssignmentRepo,
        },
        {
  ...
  it('assignRank() writes QualificationEvent with event_type: rank_changed', async () => {
    ...
    const eventCall = saveCalls.find(
      (call: any[]) =>
        call[0] === QualificationEvent &&
        call[1]?.event_type === 'rank_changed',
    );
  ...
  it('getCurrentRank() returns only the non-revoked rank assignment', async () => {
    mockRankAssignmentRepo.findOne.mockResolvedValue({
      id: 'a1',
      rank_rule_id: 'rank-2',
      revoked_at: null,
    });
    ...
    const history = [
      {
        ...
        rank_rule_id: 'rank-2',
        revoked_at: new Date(),
        assigned_at: new Date('2026-02-01'),
      },
      {
        id: 'a2',
        rank_rule_id: 'rank-3',
        revoked_at: null,
        assigned_at: new Date('2026-03-01'),
      },
    ];
