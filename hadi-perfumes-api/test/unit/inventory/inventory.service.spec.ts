...
import {
  ...
  ReservationStatus,
} from '../../../src/modules/inventory/entities/inventory-reservation.entity';
import {
  InventoryEvent,
  InventoryEventType,
} from '../../../src/modules/inventory/entities/inventory-event.entity';
import {
  InsufficientStockException,
  ReservationNotFoundException,
} from '../../../src/modules/inventory/exceptions/inventory.exceptions';
...
describe('InventoryService', () => {
    ...
    it('should complete reservation and log event if stock is available', async () => {
      ...
      mockEntityManager.query.mockImplementation(async (sql: string) => {
        ...
        if (sql.includes('SELECT'))
          return [{ id: 'inv1', available_qty: 5, total_qty: 10 }];
      ...
      mockEntityManager.save.mockImplementation(
        async (entityClass: any, data: any) => {
          if (entityClass === InventoryReservation)
            return { id: 'res1', ...data };
          return data;
        },
      );

      const res = await service.reserveStock('user1', {
        listingId: 'list1',
        qty: 5,
      });
  ...
  describe('releaseReservation', () => {
    ...
    it('should update stock, restore listing active status, and log RELEASED event', async () => {
      ...
      mockEntityManager.save.mockImplementation(
        async (_entityClass: any, data: any) => data,
      );
      mockEntityManager.create.mockImplementation(
        (_entityClass: any, data: any) => data,
      );
