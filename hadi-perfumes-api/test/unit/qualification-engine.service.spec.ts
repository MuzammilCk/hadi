...
describe('QualificationEngineService (Unit)', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(QualificationRule),
  ...
  it('evaluateUser() returns isActive: false when personal_volume rule not met', async () => {
    ...
    const result = await service.evaluateUser(
      'user-1',
      context,
      policyVersionId,
      mockEntityManager,
  ...
  it('same QualificationContext always produces the same result (determinism check)', async () => {
    ...
    const result1 = await service.evaluateUser(
      'user-1',
      context,
      policyVersionId,
      mockEntityManager,
    ...
    const result2 = await service.evaluateUser(
      'user-1',
      context,
      policyVersionId,
      mockEntityManager,
