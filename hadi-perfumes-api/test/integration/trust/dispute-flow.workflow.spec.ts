...
describe('Dispute Flow Workflow (Integration)', () => {
  ...
  beforeAll(async () => {
    ...
    buyer = await userRepo.save(
      userRepo.create({ phone: '+919999990003', status: 'active' }),
    );
    ...
    orderForEscalation = await orderRepo.save(
      orderRepo.create({
        buyer_id: buyer.id,
        subtotal: 100,
        total_amount: 100,
  ...
  it('buyer opens dispute → payout hold created', async () => {
    ...
    const holds = await holdRepo.find({
      where: { user_id: buyer.id, reason_ref_id: dispute.id, status: 'active' },
    });
  ...
  it('evidence submitted by buyer is retrievable under dispute', async () => {
    ...
    const evidences = await evidenceRepo.find({
  ...
  it('evidence submitted for wrong dispute_id → 404', async () => {
    await expect(
      disputeService.submitEvidence(uuidv4(), buyer.id, {
        file_key: 'test.jpg',
      }),
  ...
  it('admin resolves with no_action → no ResolutionEvent + hold released', async () => {
    ...
    const noActionOrder = await orderRepo.save(
      orderRepo.create({
        buyer_id: buyer.id,
        subtotal: 100,
        total_amount: 100,
    ...
    const events = await resolutionRepo.find({
  ...
  it('admin resolves with refund_granted → clawback ResolutionEvent + hold released', async () => {
    ...
    const events = await resolutionRepo.find({
      where: { entity_id: dispute!.id, resolution_type: 'clawback_triggered' },
    });
    expect(events.length).toBe(1);

    const holds = await holdRepo.find({
      where: { reason_ref_id: dispute!.id },
    });
    expect(holds.length).toBe(1);
    expect(holds[0].status).toBe('released');
  });
  ...
  it('DisputeEscalationJob → escalates overdue open disputes', async () => {
    const dispute = await disputeRepo.findOne({
      where: { order_id: orderForEscalation.id },
    });
