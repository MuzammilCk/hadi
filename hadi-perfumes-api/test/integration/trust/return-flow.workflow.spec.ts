...
import { ReturnRequest, ReturnRequestStatus, ReturnReasonCode } from '../../../src/modules/trust/returns/entities/return-request.entity';
...
import { ReturnIneligibleException, ReturnWindowExpiredException } from '../../../src/modules/trust/returns/exceptions/return.exceptions';

// Minimal mock for ClawbackJob dependency
const mockClawbackJob = {
  clawbackForOrder: jest.fn().mockResolvedValue({ processed: 1, skipped: 0, errors: 0 }),
};
...
describe('Return Flow Workflow (Integration)', () => {
  ...
  beforeAll(async () => {
    module = await Test.createTestingModule({
      imports: [
        ...
        TypeOrmModule.forFeature([
          User, Order, ReturnRequest, ReturnItem, ReturnStatusHistory, ReturnEvidence, ResolutionEvent, TrustAuditLog
        ]),
      ],
      providers: [
        ReturnService,
    ...
    buyer = await userRepo.save(userRepo.create({ phone: '+919999990001', status: 'active' }));
    const otherBuyer = await userRepo.save(userRepo.create({ phone: '+919999990002', status: 'active' }));
    ...
    orderOtherBuyer = await orderRepo.save(orderRepo.create({
    ...
    orderExpired = await orderRepo.save(orderRepo.create({
      buyer_id: buyer.id,
      subtotal: 100,
      total_amount: 100,
      currency: 'INR',
  ...
  it('buyer creates return for delivered order → status=pending_review', async () => {
    const returnReq = await returnService.createReturn(buyer.id, {
  ...
  it('admin approves return → status=approved', async () => {
    const ret = await returnRepo.findOne({ where: { order_id: orderValid.id } });
    const approved = await returnService.approveReturn(ret!.id, adminId, 'Approved');
  ...
  it('admin completes return → ResolutionEvent(clawback_triggered) written', async () => {
    const ret = await returnRepo.findOne({ where: { order_id: orderValid.id } });
    const completed = await returnService.completeReturn(ret!.id, adminId, 'Completed');
    ...
    const resolution = await resolutionRepo.findOne({ where: { entity_id: ret!.id, resolution_type: 'clawback_triggered' } });
  ...
  it('ReturnEligibilityJob picks up approved return → calls ClawbackJob', async () => {
    ...
    const orderForJob = await orderRepo.save(orderRepo.create({
      buyer_id: buyer.id,
      subtotal: 100,
      total_amount: 100,
      currency: 'INR',
    ...
    const ret = await returnService.createReturn(buyer.id, {
    ...
    const resEvent = await resolutionRepo.findOne({ where: { entity_id: ret.id, resolution_type: 'clawback_triggered' } });
  ...
  it('order outside return window → ReturnWindowExpiredException', async () => {
    await expect(returnService.createReturn(buyer.id, {
      order_id: orderExpired.id,
      reason_code: ReturnReasonCode.DAMAGED,
      idempotency_key: uuidv4(),
    }, uuidv4())).rejects.toThrow(ReturnWindowExpiredException);
  });
  ...
  it('non-owned order → ReturnIneligibleException', async () => {
    await expect(returnService.createReturn(buyer.id, {
      ...
      idempotency_key: uuidv4(),
    }, uuidv4())).rejects.toThrow(ReturnIneligibleException);
  });
});
