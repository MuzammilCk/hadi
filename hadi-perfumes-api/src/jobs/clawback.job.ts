...
export class ClawbackJob {
  ...
  async clawbackForOrder(
    orderId: string,
  ): Promise<{ clawedBack: number; skipped: number }> {
    const now = new Date();
    const events = await this.commissionEventRepo.find({
    ...
    for (const event of events) {
      if (['paid', 'clawed_back', 'voided'].includes(event.status)) {
        skipped++;
        continue;
      }
      ...
      try {
        await this.dataSource.transaction(async (em) => {
          ...
          if (!fresh || ['clawed_back', 'voided'].includes(fresh.status))
            return;
        ...
        this.logger.error(
          `Failed to claw back commission event ${event.id}:`,
          err,
        );
