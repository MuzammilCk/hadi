...
import { FraudAggregationJob } from '../../modules/trust/jobs/fraud-aggregation.job';
...
@Processor(QUEUE_NAMES.FRAUD_AGGREGATION)
@Injectable()
export class FraudAggregationProcessor {
  private readonly logger = new Logger(FraudAggregationProcessor.name);

  constructor(
    private readonly fraudAggregationJob: FraudAggregationJob,
  ...
  async handleRun(job: Bull.Job): Promise<void> {
    const run = await this.jobRunRepo.save(
      this.jobRunRepo.create({
        job_name: 'FraudAggregationJob',
        queue_name: QUEUE_NAMES.FRAUD_AGGREGATION,
    ...
    try {
      await this.fraudAggregationJob.run();
  ...
  async onFailed(job: Bull.Job, err: Error): Promise<void> {
    ...
    if (job.attemptsMade >= maxAttempts) {
      ...
      this.logger.error(
        `FraudAggregationJob dead-lettered after ${job.attemptsMade} attempts: ${err.message}`,
      );
    }
  }
}
