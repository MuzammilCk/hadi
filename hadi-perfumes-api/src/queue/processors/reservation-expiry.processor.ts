...
export class ReservationExpiryProcessor {
  ...
  async handleRun(job: Bull.Job): Promise<void> {
    const run = await this.jobRunRepo.save(
      this.jobRunRepo.create({
        job_name: 'ReservationExpiryJob',
        queue_name: QUEUE_NAMES.RESERVATION_EXPIRY,
    ...
    try {
      const result = await this.reservationExpiryJob.run();
  ...
  async onFailed(job: Bull.Job, err: Error): Promise<void> {
    ...
    if (job.attemptsMade >= maxAttempts) {
      ...
      this.logger.error(
        `ReservationExpiryJob dead-lettered after ${job.attemptsMade} attempts: ${err.message}`,
      );
    }
  }
}
