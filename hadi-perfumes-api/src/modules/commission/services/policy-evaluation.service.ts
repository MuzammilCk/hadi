...
export class PolicyEvaluationService {
   ...
   */
  evaluateEligibility(event: EvaluationEventContext, rule: CommissionRule): boolean {
    ...
    if (rule.eligible_categories && rule.eligible_categories.length > 0) {
      const hasEligibleCategory = event.productCategories.some(cat => rule.eligible_categories.includes(cat));
      if (!hasEligibleCategory) {
        return false;
      }
    }

    if (rule.eligible_seller_statuses && rule.eligible_seller_statuses.length > 0) {
   ...
   */
  isEventProhibited(eventType: 'retail_sale' | 'signup' | 'rank_upgrade' | 'self_purchase'): boolean {
