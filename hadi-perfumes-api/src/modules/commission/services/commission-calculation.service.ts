...
export class CommissionCalculationService {
  ...
  constructor(
    ...
    private outboxRepo: Repository<MoneyEventOutbox>,
    @InjectRepository(CommissionEvent)
    private commissionEventRepo: Repository<CommissionEvent>,
    @InjectRepository(CommissionEventSource)
    private sourceRepo: Repository<CommissionEventSource>,
    @InjectRepository(NetworkNode)
    private networkNodeRepo: Repository<NetworkNode>,
    @InjectRepository(QualificationState)
    private qualStateRepo: Repository<QualificationState>,
  ...
  async processOutboxEvent(outboxEvent: MoneyEventOutbox): Promise<void> {
    ...
    if (!order || order.status !== 'paid') {
      this.logger.warn(
        `Order ${payload.order_id} not in paid status — skipping`,
      );
    ...
    const rulesByLevel = new Map(
      policy.commission_rules.map((r) => [r.level, r]),
    );
    const maxLevel = parseInt(process.env.MAX_COMMISSION_LEVELS || '5', 10);

    const buyerNode = await this.networkNodeRepo.findOne({
      where: { user_id: payload.buyer_id },
    });
    if (!buyerNode) {
      this.logger.warn(
        `Buyer ${payload.buyer_id} (Order ${payload.order_id}) has no network node. Marking outbox ${outboxEvent.id} as published but no commissions generated.`,
      );
      await this.outboxRepo.update(
        { id: outboxEvent.id },
        { published: true, published_at: new Date() },
    ...
    const uplinePath: string[] =
      typeof buyerNode.upline_path === 'string'
        ? JSON.parse(buyerNode.upline_path as any)
        : buyerNode.upline_path;
    ...
    await this.dataSource.transaction(async (em) => {
      for (
        let level = 1;
        level <= Math.min(uplinePath.length, maxLevel);
        level++
      ) {
        ...
        if (
          Number(rule.min_order_value) > 0 &&
          Number(payload.total_amount) < Number(rule.min_order_value)
        )
          continue;

        // Qualification check — use em.findOne (not injected repo) to stay in transaction
        const qualState = await em.findOne(QualificationState, {
          where: { user_id: beneficiaryId },
        });
        if (!qualState?.is_qualified) continue;

        const rawAmount =
          Number(payload.total_amount) * Number(rule.percentage);
        ...
        const availableAfter = new Date(
          now.getTime() + Number(rule.payout_delay_days) * 86400000,
        );
        const clawbackBefore = new Date(
          availableAfter.getTime() +
            Number(rule.clawback_window_days) * 86400000,
        );
        ...
        try {
          ...
          if (
            ...
            this.logger.debug(
              `Commission already exists for ${idempotencyKey} — skipping`,
            );
            continue;
          }
          throw err;
        ...
        await em.save(
          ...
          em.create(CommissionEventSource, {
  ...
  async processUnpublishedEvents(): Promise<{
    processed: number;
    skipped: number;
    errors: number;
  }> {
    const batchSize = parseInt(
      process.env.COMMISSION_CALC_BATCH_SIZE || '50',
      10,
    );
