...
export class ReturnEligibilityJob {
  ...
  async processApproved(): Promise<{ processed: number; errors: number }> {
    ...
    for (const ret of approvedReturns) {
      try {
        await this.dataSource.transaction(async (em) => {
          const fresh = await em.findOne(ReturnRequest, {
            where: { id: ret.id },
          });
          if (
            !fresh ||
            fresh.status !== ReturnRequestStatus.APPROVED ||
            fresh.clawback_triggered
          )
            return;
    ...
    this.logger.log(
      `ReturnEligibilityJob: processed=${processed}, errors=${errors}`,
    );
