...
export class FraudSignalService {
  ...
  async recordSignal(params: {
    ...
    return this.dataSource.transaction(async (em) => {
      ...
      if (
        params.userId &&
        [FraudSignalSeverity.HIGH, FraudSignalSeverity.CRITICAL].includes(
          params.severity,
        )
      ) {
  ...
  private async upsertRiskAssessment(
    ...
    let assessment = await em.findOne(RiskAssessment, {
  ...
  async reviewSignal(
    ...
    return this.dataSource.transaction(async (em) => {
        ...
        { id: signalId },
        {
          status:
            verdict === 'false_positive'
              ? FraudSignalStatus.FALSE_POSITIVE
      ...
      if (verdict === 'false_positive' && signal.user_id) {
        ...
        if (holdForSignal) {
          await this.holdService.releasePayoutHold(
            holdForSignal.id,
            adminActorId,
            note ?? 'False positive — hold released',
            em,
          );
        }
      }
  ...
  async listSignals(query: FraudSignalQueryDto): Promise<{
    data: FraudSignal[];
    total: number;
    page: number;
    limit: number;
    ...
    if (query.user_id)
      qb.andWhere('fs.user_id = :userId', { userId: query.user_id });
  ...
  async getSignal(signalId: string): Promise<FraudSignal> {
    const signal = await this.fraudSignalRepo.findOne({
      where: { id: signalId },
    });
    if (!signal) throw new FraudSignalNotFoundException(signalId);
    return signal;
