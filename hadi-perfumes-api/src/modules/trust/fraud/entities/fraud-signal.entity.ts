...
export class FraudSignal {
  ...
  @Column({ type: 'varchar', length: 20, default: FraudSignalSeverity.MEDIUM }) severity: string;
  @Column({ type: 'varchar', length: 50 }) source: string;
  @Column({ type: 'simple-json', nullable: true }) evidence: Record<string, any> | null;
  @Column({ type: 'varchar', length: 255, nullable: true }) rule_ref: string | null;
  @Column({ type: 'varchar', length: 20, default: FraudSignalStatus.NEW }) status: string;
